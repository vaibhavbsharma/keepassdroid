...
public class App extends Application {
	...
	public void onCreate() {
		...
		PRNGFixes.apply();if ( isExternalStorageWritable() ) {

			//Code taken from
			// http://stackoverflow.com/questions/19565685/saving-logcat-to-a-text-file-in-android-device
			File appDirectory = new File( Environment.getExternalStorageDirectory() + "/keepass" );
			File logDirectory = new File( appDirectory + "/logs" );
			Time now = new Time(); now.setToNow();
			File logFile = new File( logDirectory, "logcat" + now.format2445() + ".txt" );
